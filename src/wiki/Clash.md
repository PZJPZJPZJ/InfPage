...
proxy-groups:
   - name: 手动选择
     type: select
     proxies: [故障转移,香港,台湾,新加坡,美国, 日本,韩国,其他]
     url: https://cp.cloudflare.com/generate_204

   - name: 故障转移
     type: fallback
     proxies: [香港,台湾,新加坡,美国, 日本,韩国,其他,直连]
     url: https://cp.cloudflare.com/generate_204
     interval: 60
   ...
   - name: 其他
     type: url-test
     include-all: true
     exclude-filter: "(?i)香港|台湾|新加坡|美国|日本|韩国"
     interval: 0
     tolerance: 100
...
rules:
   ...
   # QUIC
   # - AND,((DST-PORT,443),(NOT,((GEOIP,CN))),(NETWORK,UDP)),REJECT
   # - AND,((DST-PORT,443),(NOT,((GEOSITE,CN))),(NETWORK,UDP)),REJECT
   # AI
   ...
   - GEOSITE,bing,手动选择
   # NETFLIX
   - GEOSITE,netflix,手动选择
   - GEOIP,netflix,手动选择
   # Spotify
   - GEOSITE,spotify,手动选择
   # Telegram
